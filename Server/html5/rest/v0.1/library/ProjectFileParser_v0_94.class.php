...
require_once("Server/html5/rest/v0.1/library/ProjectFileParser_v0_93.class.php");
...
class ProjectFileParser_v0_94 extends ProjectFileParser_v0_93
{
  ...
  //global list?
  protected function findListInArray($name, $array)
  {
    foreach($array as $lst)
    {
      if($lst->name === $name)
      {
        return $lst;
  ...
  protected function getListId($name)
  {
    ...
    // global search
    $res = $this->findListInArray($name, $this->lists);
    if($res === false)
    {
      //dto to insert
      $obj = $this->currentSprite;
      //local search
      $res = $this->findListInArray($name, $obj->lists);
      if($res === false)
      {
        //not defined yet
        $id = $this->getNewId();
  ...
  //but split up into several brickObjectVariableLists in the catrobat file format
