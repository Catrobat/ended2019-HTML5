...
class ProjectFileParser
{
  ...
  protected function parseHeader()
  {
    ...
    //device 
    $device = new ProjectDeviceDto(intval($xmlh->screenHeight), intval($xmlh->screenWidth),
                                   (string)$this->getProperty($xmlh, "screenMode"));

    //header
    $header = new ProjectHeaderDto((string)$xmlh->programName, (string)$xmlh->description,
                                   floatval($xmlh->catrobatLanguageVersion), (string)$xmlh->userHandle,
                                   (string)$xmlh->url);
  ...
  protected function parseSprite($sprite, $spriteId)
  {
    ...
    foreach($sprite->lookList->children() as $look)
    {
      ...
      {
        $id = $this->getNewId();
        $size = filesize($this->cacheDir . "images" . DIRECTORY_SEPARATOR
                         . (string)$look->fileName);  //str_replace("/", DIRECTORY_SEPARATOR,
    ...
    foreach($sprite->soundList->children() as $sound)
    {
      ...
      $res = $this->findResourceInArray("sounds" . DIRECTORY_SEPARATOR . (string)$sound->fileName,
                                        $this->sounds); //= false, if not found
      if($res === false)
      {
        $id = $this->getNewId();
      ...
      while($idx < $length)
      {
        ...
        switch($this->getBrickName($script))
        {
          ...
          case "ForeverBrick":      //"loopEndlessBrick"
            ...
            $nestedCounter
              = 0;   //use a counter ob nested elements with same name as comparison of objects using equal or operator (===) is not available in simpleXML
            $idx++;
  ...
  protected function parseFormula(/*formulaTree*/
    $formula)
  {
