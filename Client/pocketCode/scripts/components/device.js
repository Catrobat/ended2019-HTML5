...
PocketCode.MediaDevice = (function () {
    ...
    MediaDevice.prototype.merge({
        ...
        __detectFace: function () { //should not be called directly (even  internal)
            ...
            var gsImg = Uint8Array ? new Uint8Array(w * h) : new Array(w * h),
            ...
            var cMap = new Array(w),
                colSum = Uint8Array ? new Uint8Array(w) : new Array(w),
                row, sum, p;

            var _r, _g, _b;
            for (var c = 0, cl = w; c < cl; c++) {
                row = Uint8Array ? new Uint8Array(h) : new Array(h);
                sum = 0;
            ...
            for (var i = 0; i < roiLength && !face; i++) {
                roi = rois[i];
                ...
                else {  //use the full canvas
                    hData = ctx.getImageData(0, 0, w, h).data;
                }
